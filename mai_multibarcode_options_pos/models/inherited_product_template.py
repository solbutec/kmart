...
class ProductTemplate(models.Model):
	...
	allow_multi_barcodes = fields.Boolean('Allow Multiple Barcodes',default=True)
...
class ProductMultiBarcode(models.Model):
	...
	_sql_constraints = [
		('brcd_name_uniq', 'unique (barcode)', 'Barcode should be unique.')
	]
...
class ProductProduct(models.Model):
	...
	def get_multi_barcode_product(self,code):
		prod = False
		barcode = self.env['product.multi.barcode'].search([('barcode','=',code)])
		if barcode:
			prod = barcode.product_id
			if not prod and barcode.product_tmpl_id:
				prod = barcode.product_tmpl_id.product_variant_id
			return prod.id
		else:
			prod = self.env['product.product'].search([('barcode','=',code)])
			return prod.id
	...
	def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
		...
		if name:
			...
			if multi_barcode_ids:
				allow_multi_barcodes_ids = self._search(['|',('allow_multi_barcodes', '=', True),('product_tmpl_id.allow_multi_barcodes', '=', True)], limit=limit)
				if allow_multi_barcodes_ids:
					product_ids = product_ids + self._search([
						'|',
						('multi_barcode_ids', 'in', multi_barcode_ids),
						('product_tmpl_id.multi_barcode_ids', 'in', multi_barcode_ids)
